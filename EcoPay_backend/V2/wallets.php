...
class Wallet {
    ...
    public function createWallet($userId, $walletName, $currency = 'USD') {
        try {
            $stmt = $this->pdo->prepare("INSERT INTO Wallets (user_id, wallet_name, currency, is_default) VALUES (?, ?, ?, (SELECT COUNT(*) = 0 FROM Wallets WHERE user_id = ?))");
            $stmt->execute([$userId, $walletName, $currency, $userId]);
